    ...
    (async () => {
      try {
        ...
        try {
          ...
          console.error('Erreur chargement articles membre :', error);
          setArticles([]);
        }
      } catch {
        navigate('/connexion-membre', { replace: true });
  ...
  async function handleDeleteArticle(articleId) {
    try {
      ...
      if (code === 401 || code === 403) {
        ...
        setMessage('❌ Erreur suppression article.');
      }
    } finally {
  ...
  return (
    <section
      ...
      <Modal
        ...
        cancelText="Annuler">
        Cette action supprimera définitivement votre compte membre.
      </Modal>
      ...
      <Modal
        ...
        onConfirm={() => handleDeleteArticle(articleToDelete)}
        confirmText="Supprimer"
        cancelText="Annuler">
        Cette action supprimera définitivement l’article sélectionné.
      </Modal>

      <h2 id="member-profile-title">👤 Profil Membre</h2>
        ...
        <>
          ...
          <ul className="actions-list">
            ...
            <li className="actions-item">
              <Link
                to="/ajouter"
                className="button-link btn btn--with-text"
      ...
      <section aria-labelledby="member-articles-title">
        <h3 id="member-articles-title">📝 Mes articles publiés</h3>

        {articles.length > 0 ? (
          <ul className="articles-list">
            {articles.map(a => (
              <li key={a.article_id} className="articles-item">
                <article aria-label={`Article ${a.title}`}>
                  <h4>{a.title}</h4>
                  ...
                  <ul className="item-actions">
                    ...
                    <li>
                      <button
                        ...
                        className="remove btn btn--with-text"
                        onClick={() => {
                          setArticleToDelete(a.article_id);
